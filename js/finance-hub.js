/**
 * Finance Hub - News Dashboard
 */

const DATA_URL = '../data/finance-news.json';
...
function renderArticles() {
  ...
  grid.innerHTML = articles.map(article => `
    <article class="news-card">
      ...
      <div class="news-meta">
        <span class="news-category ${categoryColors[article.category] || ''}">${escapeHtml(article.category)}</span>
      </div>
    </article>
  `).join('');
}
  ...
  try {
    ...
    grid.innerHTML = `
      <div class="empty-state" style="grid-column: 1 / -1;">
        ...
        <p style="font-size: 0.8rem; margin-top: 8px;">Run fetch-finance.py first or wait for GitHub Actions.</p>
      </div>
    `;
  }
}
